...
protected:
  enum BitWidths {
    ...
    kNEntryPerBin = 1<<(NBIT_ADDR-NBIT_BIN)
  };
...
public:
  ...
  unsigned int getNEntryPerBin() const {return kNEntryPerBin;}
  ...
  void clear()
  {
    ...
    for (size_t ibx=0; ibx<kNBxBins; ++ibx) {
      for (size_t ibin=0; ibin<kNSlots; ++ibin) {
        ...
        for (size_t addr=0; addr<kNEntryPerBin; ++addr) {
